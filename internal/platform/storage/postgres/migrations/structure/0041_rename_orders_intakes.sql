...
COMMENT ON COLUMN public.intakes.status     IS 'Estado del ciclo de vida EXTENDIDO (D-041.10, ADR-0031). Claves canónicas: "open" | "pending_approval" | "needs_info" | "confirmed" | "deposit_requested" | "deposit_paid" | "settled" | "cancelled" | "rejected" | "abandoned". "abandoned" es terminal y significa que la solicitud dejó de estar viva sin que nadie la resolviera: se canceló el evento que la sostenía (Plan 043) o el dueño descartó el pedido huérfano (D-041.18). Conviven DOS claves legadas que no se migran: "closed" —la que el módulo cart escribe desde el Plan 016— se lee como "confirmed", y "expired" quedó del reloj derogado (D-041.16), en la que ya nadie entra. Sin CHECK a propósito: la validación vive en la máquina de estados del código (internal/intakes/status.go), no en la tabla; un CHECK obligaría a migrar por cada estado nuevo y dejaría fuera las filas históricas.';
