...
// ejercer la proyección del PersistSink (intakes/intake_items) y, en paralelo, el
// WebhookSink stub.
...
func runCartClosed(t *testing.T, extra ...runtime.Option) *store.MemoryRepository {
	...
	opts := append([]runtime.Option{
		runtime.WithEventSink(runtime.NewPersistSink(repo, cart.NewProjector(repo, intakes.NewMemoryStore(), sinEnvío{}, intakes.NewMemoryStore()))),
	}, extra...)
...
func TestWebhookSink_Registrado_NoAlteraPersistSink(t *testing.T) {
	...
	oa, ob := repoSolo.Intakes(), repoConWebhook.Intakes()
	if len(oa) != 1 || len(ob) != 1 {
		t.Fatalf("intakes debe ser 1 en ambas: solo=%d con-webhook=%d", len(oa), len(ob))
	}
	if oa[0].Status != ob[0].Status || oa[0].Total != ob[0].Total {
		t.Fatalf("solicitud difiere: solo=%+v con-webhook=%+v", oa[0], ob[0])
	}
	if got := len(repoSolo.IntakeItems(oa[0].ID)); got != 1 || got != len(repoConWebhook.IntakeItems(ob[0].ID)) {
		t.Fatalf("intake_items difieren o no es 1: solo=%d con-webhook=%d", got, len(repoConWebhook.IntakeItems(ob[0].ID)))
	}
}
